...
const QuizPlay = () => {
  ...
  useEffect(() => {
    const start = async () => {
      try {
        const result = await axios.get("https://railway-dev-production-d537.up.railway.app/front/getstart");
  ...
  const fetchquestion = async() =>{
    const result = await axios.get("https://railway-dev-production-d537.up.railway.app/question/getquestion");
      console.log(result);
  ...
  const handleAnswerButtonClick = async (selectedOption) => {
    ...
    if (selectedOption === currentQuestion.correctAnswer) {
      ...
      if (isNameJoined) {
        try {
          const userResponse = await axios.get(`https://railway-dev-production-d537.up.railway.app/quiz/getUserIdByNickname/${targetGamepin}/${name}`);
